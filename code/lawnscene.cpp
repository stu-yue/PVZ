...
void lawnScene::mouseMoveEvent(QMouseEvent* event)
{
    grid = event->pos();
...
void lawnScene::mousePressEvent(QMouseEvent *event)
{
...
void lawnScene::plantPlants(QPoint grid)
{
    Plants* plant;
...
//        break;
    ...
    sunShine -= this->currentCard->sunPoint;
    if(currentCard!=nullptr)
        delete currentCard;
    this->currentCard = nullptr;
}
...
void lawnScene::putZombie(int row, int type)
{
    Zombie* zombie;
    switch(type)
    {
...
void lawnScene::setUI()
{
    ...
    sunNum->setText(QString::number(sunShine));
...
//    Card *card_8 = new MushroomCard(this);
//    card_8->setIndex(8);
//    cardSet.append(card_8);
//    Card *card_9 = new MushroomCard(this);
//    card_9->setIndex(0);
//    cardSet.append(card_9);
...
lawnScene::lawnScene(QWidget *parent) : QWidget(parent),counter(0)
{
    setMouseTracking(true);
    //configure basic information
    timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &lawnScene::onTimer);
    timer->start(20);
    setUI();

}

void lawnScene::paintEvent(QPaintEvent *event){
    QPainter painter(this);
    //QPixmap pix(":/res/background1.jpg");
    QImage backImg(":/res/background1.jpg");
    painter.drawImage(0,0,backImg,120,0);
}

void lawnScene::remove()
{
    int p = 0;
    //plant
